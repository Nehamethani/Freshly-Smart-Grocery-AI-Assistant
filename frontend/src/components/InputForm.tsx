...
const activityLevels = [
  ...
  {
    value: "Extra active (very hard exercise, physical job)",
...
const UpdateProfileForm = () => {
  ...
  const validateStep1 = () => {
    ...
    if (
      Number.parseInt(profileData.age) < 13 ||
      Number.parseInt(profileData.age) > 120
    ) {
      setError("Please enter a valid age between 13 and 120");
      return false;
    }
    if (
      Number.parseInt(profileData.height) < 100 ||
      Number.parseInt(profileData.height) > 250
    ) {
      setError("Please enter a valid height between 100-250 cm");
      return false;
    }
    if (
      Number.parseInt(profileData.weight) < 30 ||
      Number.parseInt(profileData.weight) > 300
    ) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    const response = axios
      .put(`/api/update/${email}`, JSON.stringify(profileData))
      .then((response) => {
        console.log("Response:", response.data);
      })
      .catch((error) => {
    ...
    setSuccess(
      "Profile updated successfully! Redirecting to your user dashboard..."
    );
      ...
      {currentStep === 2 && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Help us understand your food preferences and restrictions
            </p>
          </CardHeader>
          <CardContent className="space-y-6">
            <div>
              ...
              {profileData.dietInfo.cuisines.length > 0 && (
                <div className="mt-2">
                  <p className="text-sm text-gray-600">
                    Selected: {profileData.dietInfo.cuisines.length} cuisines
                  </p>
                </div>
              )}
            </div>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
                {allergyOptions.map((allergy) => (
                  <Button
                    ...
                    type="button"
                    variant={
                      profileData.dietInfo.allergies.includes(allergy)
                        ? "default"
                        : "outline"
                    }
                    size="sm"
      ...
      {currentStep === 3 && (
        <Card>
          <CardHeader>
            ...
            <p className="text-sm text-gray-600">
              Tell us about your specific food likes and dislikes
            </p>
          </CardHeader>
          <CardContent>
              ...
              <div>
                ...
                <Textarea
                  ...
                  onChange={(e) =>
                    setProfileData((prev) => ({
                      ...prev,
                      dietInfo: { ...prev.dietInfo, likedFood: e.target.value },
                    }))
                  }
                  rows={4}
                />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                  ...
                  <div>
                    <p>
                      ...
                      {profileData.dietInfo.dietType}
                    </p>
                    <p>
                      <strong>Cuisines:</strong>{" "}
                      {profileData.dietInfo.cuisines.length} selected
                    </p>
                    <p>
